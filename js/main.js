...
(function(){
	...
	function connect() {
		...
		try {
			socket.onopen = function (msg) { //连接成功建立的回调
	...
	$('.fa-group').click(function(){
		$('.operations').hide()  //对原始默认的界面先隐藏
		$('#groupop').show()  //对仅内容不一样的界面显示
		msg={'type':'getGroups','username':username}
		msg=JSON.stringify(msg);
		socket.send(msg); //发送获取群组的请求
	...
	$('#addGroup').click(function(){
		...
		$('#groupAddYes').click(function(){
			...
			var groupMems = ($('#groupMembers').val()).slice(0)
